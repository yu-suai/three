	...
	parse(data) {
		...
		function parseXML(stringFile) {
			...
			if (json.PolyData) {
				...
				console.log('VTU add UnstructuredGrid!')
				...
				try {
					while (sectionIndex < numberOfSections) {
						...
						if (section && section.DataArray) {
						  ...
						  // 重点修改：处理Cells数据
							...
							numberOfCells = parseInt(piece.attributes.NumberOfCells);
							if (numberOfCells > 0) {
							  for (let i = 0; i < arr.length; i++) {
								const dataArr = arr[i];
								const name = dataArr.attributes.Name ?? '';
								
								if (name === 'connectivity') {
								  connectivity = dataArr?.text ?? [];
								} else if (name === 'offsets') {
								  offsets = dataArr?.text ?? [];
								} else if (name === 'types') {
								  types = dataArr?.text ?? [];
								}
							  }
					...
					// 生成网格索引（关键修改）
					if (connectivity && offsets && types && numberOfCells > 0) {
					indices = [];
					let offsetIndex = 0;
					let startIndex = 0;
					...
					for (let i = 0; i < numberOfCells; i++) {
						...
						const vertexCount = endIndex - startIndex;
			
						// 仅处理三角形(5)和四边形(9)
						if (cellType === 5) { // VTK_TRIANGLE
						if (vertexCount === 3) {
							indices.push(
							connectivity[startIndex],
							connectivity[startIndex + 1],
							connectivity[startIndex + 2]
							);
						}
						} else if (cellType === 9) { // VTK_QUAD
						if (vertexCount === 4) {
							// 将四边形分解为两个三角形
							indices.push(
							connectivity[startIndex],
				...
				console.log('end', json, indices, points)
